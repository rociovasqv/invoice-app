...
import { URL_FACTURAS_COMPRA_EDITAR, URL_FACTURAS_COMPRA_FILTRAR } from "../constants/constantes";

const useEditarCompraForm = () => {
  ...
  const navigate = useNavigate();
    ...
    const fetchFactura = async () => {
      try {
        const response = await axios.get(URL_FACTURAS_COMPRA_FILTRAR+"/"+id);
...
useEffect(()=>{
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.put(URL_FACTURAS_COMPRA_EDITAR+"/"+id, {
      ...
      alert("Factura de compra actualizada correctamente");
      navigate("/compras");
    } catch (error) {
...
export default useEditarCompraForm;